    ...
    return (
        <div>
            ...
            >
                ...
                >
                <Dialog.Panel className="fixed right-0 top-0 h-full w-full max-w-full sm:max-w-md bg-white shadow-xl p-4 sm:p-6 overflow-y-auto">
                    {kycStatus === "noKYC" ? (
                        <KYCForm user={user} setKycStatusAction={setKycStatus} onClose={() => setShowConsultation(false)} />
                    ) : kycStatus === "pending" ? (
                        <PendingKYC onClose={() => setShowConsultation(false)} />
                    ) : (
                        <ConsultationScheduler user={user} supabase={supabase} onClose={() => setShowConsultation(false)} />
                    )}
                </Dialog.Panel>
                </Transition.Child>
            </Dialog>
